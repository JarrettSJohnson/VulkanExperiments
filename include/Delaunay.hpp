...
{
public:
  RandomCoordGenerator(std::size_t numPts)
  {
...
private:
  ...
  std::vector<glm::vec3> generateRandom2DPoints(std::size_t numPts)
  {
      ...
      m_hash_size(), m_edge_stack()
{
  ...
  for (std::size_t k = 0; k < n; k++) {
    ...
    while (q = hull_next[next],
        orient(x, y, coords[2 * next], coords[2 * next + 1], coords[2 * q],
            coords[2 * q + 1])) {
    ...
    if (e == start) {
      while (q = hull_prev[e], orient(x, y, coords[2 * q], coords[2 * q + 1],
